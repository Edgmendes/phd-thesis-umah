...
ds_virtual = 0.5e-3;        % [m]
dt = 1e-5;                  % [s]   note: 1e-4 avoids rounding problems related w temporal shifts
...
for i = 1 : nFPs
    ...
    [spatial_temporal_signal] = Generate_STM_signal(T_draw, F_mod, N_SP, L, ...
    ...
    % AUX variables
    T_draw_aux(i) = T_draw;
    stimulus_type_aux = strcat(stimulus_type_aux, stimulus_type);

    if i < nFPs
        stimulus_type_aux = strcat(stimulus_type_aux,'&');
    end
...
[magnitudes, phases] = Rescale_cast_data(spatial_temporal_signal, n_bits);
...
params = 't_samples - ' + string(size(spatial_temporal_signal,3)) + '; Nsource - ' + string(N_source) ...
        ...
        + '; Stimulus - ' + stimulus_type_aux + '; Amax_FP - ' + A_max + '; nFPs - ' + nFPs ...
        + '; Word Bits - ' + word_bits;
