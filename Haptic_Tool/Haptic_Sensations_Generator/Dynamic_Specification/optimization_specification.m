function [v_fp, N_SP, Fmod, tp] = optimization_specification(stimulus_type, v_fp, L, rationON)
...
% with the temporal frequency of modulation and Number of positions (N).
...
if ( (v_fp >= v_min && v_fp <= v_max) && (L >= l_min && L <= l_max) && (tp >= tif_critical) )
    disp(" Especificação Correta ");
    radious = L / (2*pi);
    state = true;

    X = ['v',' : ',num2str(v_fp), newline ,'l',' : ',num2str(L), newline , ...
